(() => {
  ...
  let activeId = localStorage.getItem(tasteKey) || "motion";
  if (!payload.directions.some((item) => item.id === activeId)) activeId = "motion";
  ...
  function cover(post, direction, compact = false) {
    ...
    return `<article class="identity-cover identity-cover-${escapeHtml(post.layout)}${compact ? " is-compact" : ""}">
  ...
  function renderRail() {
    ...
    </button>`).join("") + '<span class="identity-rail-cue" aria-hidden="true">листайте →</span>';
  }
  ...
  function renderDetail(direction) {
    ...
    ui.detail.innerHTML = `<div class="identity-detail-head">
      ...
      <div class="identity-palette" aria-label="Палитра направления">${direction.palette.map((color) => `<i style="--swatch:${color}" title="${color}"></i>`).join("")}</div>
    </div>
  ...
  function renderCompare() {
    ui.compare.innerHTML = payload.directions.map((direction) => `<button type="button" class="identity-compare-card${direction.id === activeId ? " is-active" : ""}" data-identity-direction="${escapeHtml(direction.id)}" aria-pressed="${direction.id === activeId}">
  ...
  fetch(payload.profile.source)
    ...
    .catch(() => {
      ui.profileStatus.textContent = "Пять систем доступны · профиль не удалось перепроверить";
