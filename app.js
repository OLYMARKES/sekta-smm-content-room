(() => {
  ...
  function filteredLibrary() {
    ...
    const time = (item, field) => Date.parse(item[field] || "") || 0;
    if (ui.librarySort.value === "capture-desc") return [...filtered].sort((a, b) => time(b, "captureDate") - time(a, "captureDate"));
    if (ui.librarySort.value === "capture-asc") return [...filtered].sort((a, b) => time(a, "captureDate") - time(b, "captureDate"));
    if (ui.librarySort.value === "modified-desc") return [...filtered].sort((a, b) => time(b, "modifiedAt") - time(a, "modifiedAt"));
    return filtered;
  }
  ...
  ui.librarySort.addEventListener("change", () => renderLibrary(true));
