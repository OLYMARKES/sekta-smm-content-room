(() => {
  ...
  function openMedia(item) {
    ...
    const verifiedOriginalUrl = item.originalResolution?.remoteUrl || item.originalUrl || "";
    const originalDownloadUrl = verifiedOriginalUrl || item.exportImage || item.thumb || "";
    ...
    const originalActionName = verifiedOriginalUrl ? `${safeBaseName}-original.${extension}` : `${safeBaseName}-public.${extension}`;
    const originalAction = originalDownloadUrl ? `<a class="button button-primary" href="${escapeHtml(originalDownloadUrl)}" download="${escapeHtml(originalActionName)}" data-download-original="true">Скачать ${verifiedOriginalUrl ? "оригинал" : "доступный файл"}</a>` : "";
    ...
    const sourceNote = localPathAvailable
      ...
      : `<div class="path-box">${verifiedOriginalUrl ? "HQ-оригинал подключён к публичной медиатеке." : "HQ-оригинал не подключён к публичной версии; кнопка скачивает лучший доступный файл."}</div>`;
    ...
    const originalStatus = ({ "verified-local": "Локальный оригинал проверен", "verified-local-and-remote": "Локальный и Drive-оригинал", "remote-only": "Только удалённый оригинал", unresolved: "HQ-оригинал не подключён" })[item.originalResolution?.status] || "Не проверено";
    ...
    ui.dialogContent.innerHTML = `<div class="detail-layout"><div class="detail-image">${media}</div><div class="detail-copy"><p class="eyebrow">${escapeHtml(item.folderLabel)}${category}</p><h2>${escapeHtml(item.fileName)}</h2><p>В интерфейсе используется превью; финальный экспорт берёт HQ, если оригинал подключён к публичной медиатеке.</p><div class="meta-list"><div class="meta-row"><span>Тип</span><strong>${escapeHtml(typeLabels[item.materialType] || item.materialType || "Материал")}</strong></div><div class="meta-row"><span>Публикация</span><strong>${escapeHtml(statusLabels[item.publicationStatus] || item.publicationStatus || "Не указан")}</strong></div><div class="meta-row"><span>Дата съёмки</span><strong>${escapeHtml(captureDate)}</strong></div><div class="meta-row"><span>Камера</span><strong>${escapeHtml(camera)}</strong></div><div class="meta-row"><span>Оригинал</span><strong>${escapeHtml(originalStatus)}</strong></div><div class="meta-row"><span>Размер</span><strong>${item.width} × ${item.height}</strong></div><div class="meta-row"><span>Ориентация</span><strong>${orientationLabel(item.orientation)}</strong></div><div class="meta-row"><span>Вес оригинала</span><strong>${item.sizeMb} МБ</strong></div><div class="meta-row"><span>SHA-256</span><strong>${escapeHtml(item.sha256 ? item.sha256.slice(0, 12) + "…" : "не рассчитан")}</strong></div><div class="meta-row"><span>Точные дубли</span><strong>${duplicateText}</strong></div></div>${projects}${people}${themes}${roles}<div class="detail-actions">${originalAction}${previewAction}${topAction}${addAction}${copyAction}</div>${sourceNote}</div></div>`;
