(() => {
  ...
  function openMedia(item) {
    ...
    const downloadUrl = item.originalResolution?.remoteUrl || item.originalUrl || item.exportImage || item.thumb || "";
    const downloadIsOriginal = Boolean(item.originalResolution?.remoteUrl || item.originalUrl);
    const extensionMatch = String(downloadUrl).match(/\.([a-z0-9]+)(?:[?#].*)?$/i);
    ...
    const downloadName = `${safeBaseName}-${downloadIsOriginal ? "original" : "preview"}.${extension}`;
    const downloadAction = downloadUrl ? `<a class="button button-primary" href="${escapeHtml(downloadUrl)}" download="${escapeHtml(downloadName)}" data-download-original="${downloadIsOriginal ? "true" : "false"}">${downloadIsOriginal ? "Скачать оригинал" : "Скачать превью"}</a>` : "";
